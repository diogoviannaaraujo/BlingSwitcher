...
class App (awn.AppletSimple):
  ...
  def __init__ (self, uid, orient, height):
    ...
    self.ObjSwitcher.switcher = CairoWidgets_BlingSwitcher.BlingSwitcher()
  ...
  def button_press (self, widget, event):
    if self.dialog_visible != True:
      ...
      self.dialog.hide()
      self.ObjSwitcher.switcher.destroy()
...
class Switcher:
  ...
  def GetActiveWorkspaceNumber(self):
    ...
    rows = wrkspace.get_height()/scr.get_height()
    cols = wrkspace.get_width()/scr.get_width()
    nviewp = (wrkspace.get_viewport_x() + scr.get_width())/scr.get_width()
    mviewp =  wrkspace.get_viewport_y()/scr.get_height()*cols

    return nviewp+(rows-1)*mviewp
